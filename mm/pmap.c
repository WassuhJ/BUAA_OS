...
void mips_detect_memory()
{
	...
	maxpa = (1 << 26);		// 64 MB = 2 ^ 26 B = 0x4000000
	basemem = (1 << 26);
	extmem = 0;
...
void page_init(void)
{
	...
	for (now = pa2page(PADDR(freemem)); page2ppn(now) < npage; now++) {
		...
		/*if (page2pa(now) != PADDR(TIMESTACK - BY2PG)) {
			now->pp_ref = 0;
			LIST_INSERT_HEAD(&page_free_list, now, pp_link);
		} else {
			now->pp_ref = 1;
...
int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	*ppte = ((Pte*)PTE_ADDR(KADDR(*pgdir_entry))) + PTX(va);	/*Kernel Virtual Address of PTBase */
	return 0;
}
